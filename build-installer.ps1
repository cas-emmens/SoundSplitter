<#
  ...
  Run from the repo root:   .\build-installer.ps1
  Switches:
...
$payload = Join-Path $root "src-tauri\payload"
...
# --- 2. Python dependencies (CPU torch first, then the rest) ------------------
Step "Installing Python dependencies (CPU torch + requirements + tab libs)"
& $py -m pip install torch torchaudio --index-url https://download.pytorch.org/whl/cpu
if ($LASTEXITCODE -ne 0) { Die "CPU PyTorch install failed." }
...
& $iscc "/DAppVersion=$version" (Join-Path $root "sound-splitter.iss")
if ($LASTEXITCODE -ne 0) { Die "Inno Setup compilation failed." }
$installer = Join-Path $root "dist\SoundSplitter-Setup-$version.exe"
    ...
    $latest = [ordered]@{
        ...
        platforms = [ordered]@{
            "windows-x86_64" = [ordered]@{
                signature = $sig.Trim()
                url       = "https://github.com/cas-emmens/SoundSplitter/releases/download/v$version/SoundSplitter-Setup-$version.exe"
            }
        }
    }
